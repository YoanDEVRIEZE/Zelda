...
import { findByName } from "../tools/function";
import '../tools/overwrite';
...
export default class Animator {
    ...
    initListeners() {
        this.mixer.addEventListener('loop', ({ action }) => {
            this.fireListener(action._clip.name, 'loop')
        })
        this.mixer.addEventListener('half', ({ action }) => {
            this.fireListener(action._clip.name, 'half')
        })
        this.mixer.addEventListener('start', ({ action }) => {
            this.fireListener(action._clip.name, 'start')
        })
    }
